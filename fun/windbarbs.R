...
wind_barb <- function(data, coord, panel_params, skip.x, skip.y) {
  ...
  grob.list <- lapply(seq_len(n), function(i){
    ...
    # Build geometries
    geom <- c(rep(list(g50), n50[i]), rep(list(g10), n10[i]), rep(list(g5), n5[i]))
    ...
    grob.barbs <- lapply(seq_along(geom), function(j){
      ...
      pathGrob(x = x3, y = y3, gp = gpar(col = col[i], fill = fill[i], lwd = size[i], linejoin = "mitre", lineend = "square"))
    })
    ...
    if(mag[i] > 0){
      ...
      x3 <- unit(x[i], "npc") + unit(x2, "snpc")
      y3 <- unit(y[i], "npc") + unit(y2, "snpc")
      grob.segment <- pathGrob(x3, y3, gp = gpar(col = col[i], lwd = 1*size[i], linejoin = "mitre", lineend = "square"))
    } else {
      grob.segment <- nullGrob()
    }

    # Build point grob
    grob.point <- pointsGrob(x[i], y[i], pch = 21, size = unit(point.size, "snpc"), gp = gpar(col = col[i], fill = fill[i], lwd = size[i]))
...
GeomWindBarb <- ggplot2::ggproto("GeomWindBarb", ggplot2::Geom,
  required_aes = c("x", "y"),
  ...
  draw_panel =  function(data, panel_params, coord, grob, debug, skip.x, skip.y) {
    if (!is.null(debug)) {
        coords <- coord$transform(data, panel_params)
        debug(data, coords)
    }
    if (grid::is.grob(grob)) {
        ...
        if (!is.function(grob))
            stop("Invalid grob in GeomWindBarb")
...
#' @export
#'
geom_windbarb <- function(mapping = NULL, data = NULL, stat = "identity",
    position = "identity", inherit.aes = TRUE, show.legend = FALSE,
    key_glyph = NULL, debug = NULL, skip.x = "auto", skip.y = "auto", skip.border = FALSE, ...){

  # Skip rows/cols near border (to be implemented)
  # if(skip.border)

  layer(geom = GeomWindBarb, mapping = mapping, data = data,
        stat = stat, position = position, inherit.aes = inherit.aes,
        check.aes = FALSE, show.legend = show.legend, key_glyph = key_glyph,
        params = list(grob = wind_barb, debug = debug, skip.x = skip.x, skip.y = skip.y, ...))
}
...
data <- data.frame(x = xy[,1], y = xy[,2], angle =  rep(seq(-90, 0, length.out = n), n),
                   mag = rep(seq(0, 227, length.out = n2)), group = rep(1:1, n))
# Use group = rep(c(1, 2), n/2) to show two panels

# Demo plot
pl <- ggplot(data, mapping = aes(x, y)) +
  facet_wrap(~group) +
  geom_windbarb(aes(mag = mag, angle = angle), data = data[], length = unit(3, "cm"),
    skip.x = "auto", skip.y = "auto", size = 1, fill = "#cccccc", colour = "black") +
